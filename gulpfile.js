...
gulp.task('clean-old', function() {
    return gulp.src(['build/*.min*.js','build/**/*.css'], {read: false})
    .pipe(clean());
});
...
//Start a web server on port 8282 to server the src src
...
gulp.task('build', ['clean-old', 'create-templates', 'inject-templates', 'usemin', 'add-dependencies', 'copy-asset-files', 'clean']);
